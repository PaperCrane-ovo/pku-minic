...
use koopa::ir::{dfg::DataFlowGraph, BasicBlock, BinaryOp, FunctionData, Program, Value, ValueKind};

use crate::utils::is_const;

use super::{imm::i12, register::RegId, riscv::{self, Block, Function, RiscvInst, TempRiscv}, stack::StackFrame};

use miette::Result;

type AsmProgram = Vec<TempRiscv>;
...
pub trait GenerateAsmValue{
    fn load_imm(&self, _asm: &mut AsmProgram,_dfg: &DataFlowGraph,_dest_reg:RegId)-> Result<RegId>{Ok(RegId::T0)}
    fn generate(&self,_asm: &mut AsmProgram,_dfg: &DataFlowGraph,_stack_frame:&mut StackFrame){}
    fn load_value(&self,_asm: &mut AsmProgram,_dfg: &DataFlowGraph,_stack_frame:&mut StackFrame,_dest_reg:RegId) -> Result<RegId>{
        Ok(RegId::T0)
    }
    fn store_value(&self,_asm: &mut AsmProgram,_dfg: &DataFlowGraph,_stack_frame:&mut StackFrame,_src_reg:RegId){}
}
...
impl GenerateAsm for FunctionData{
    fn generate (&self,asm: &mut AsmProgram){
        ...
        asm.push(TempRiscv::Inst(RiscvInst::Addi(RegId::SP, RegId::SP, i12::build(stack_size as i32).unwrap())));
...
impl GenerateAsmValue for Value{
    fn generate (&self,asm: &mut AsmProgram,dfg: &DataFlowGraph,stack_frame: &mut StackFrame) {
        ...
        match value_data.kind(){
            ...
            ValueKind::Return(ret) => {
                ...
                asm.push(TempRiscv::Inst(RiscvInst::Addi(RegId::SP, RegId::SP, i12::build(stack_frame.size as i32).unwrap())));
            ...
            ValueKind::Branch(branch) => {
                ...
                let true_block_label_name = dfg.bb(true_block).name().as_deref().unwrap().to_string();
                let false_block_label_name = dfg.bb(false_block).name().as_deref().unwrap().to_string();
    ...
    fn load_imm(&self,asm: &mut AsmProgram,dfg: &DataFlowGraph,dest_reg:RegId) -> Result<RegId>{
        let value_data = dfg.value(*self);
        match value_data.kind(){
            ValueKind::Integer(i) => {
                // TODO: 0的处理
    ...
    fn load_value(&self,asm: &mut AsmProgram,dfg: &DataFlowGraph,stack_frame: &mut StackFrame,dest_reg:RegId) -> Result<RegId> {
        let value_data = dfg.value(*self);
        if is_const(value_data){
            ...
            asm.push(TempRiscv::Inst(RiscvInst::Lw(dest_reg,i12::build(offset as i32).unwrap(),RegId::SP)));
            Ok(dest_reg)
        }
        
    }    
    fn store_value(&self,asm: &mut AsmProgram,_dfg: &DataFlowGraph,stack_frame:&mut StackFrame,src_reg:RegId) {
        if !stack_frame.contains(*self){
            stack_frame.insert(*self);
        }
        let offset = stack_frame.get(*self);
        // asm.push_str(&format!("    sw {}, {}(sp)\n",src_reg,offset));
        asm.push(TempRiscv::Inst(RiscvInst::Sw(src_reg,i12::build(offset as i32).unwrap(),RegId::SP)));
    }
}